...
function RgbHslCmykInput:get_color(index)
	local v = self.value
	if index == 1 then -- R
		return { v[1], 0, 0 }
	elseif index == 2 then -- G
		return { 0, v[2], 0 }
	elseif index == 3 then -- B
		return { 0, 0, v[3] }
	elseif index == 4 then -- H
		return convert.hsl2rgb({ v[4], 1, 0.5 })
	elseif index == 5 or index == 6 then -- S/L
		return convert.hsl2rgb({ v[4], v[5], v[6] })
	elseif index >= 7 then -- CMYK
		return convert.cmyk2rgb({ v[7], v[8], v[9], v[10] })
	end
end
